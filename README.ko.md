...
| **`bash`**가 `PATH`에 있을 것 | 모든 hook과 셋업 로직은 POSIX bash로 작성되어 있습니다. 네이티브 Windows(PowerShell / cmd)는 **지원하지 않습니다** — WSL2 또는 Git Bash를 사용하세요 |
| **`jq`**가 `PATH`에 있을 것 | Stop hook이 트랜스크립트 JSONL과 상태 파일 JSON을 파싱할 때 사용합니다 |
| **`claude` CLI**가 `PATH`에 있을 것 | headless Haiku 분류 호출에 사용됩니다. 인증이 완료되어 있어야 합니다 (OAuth 또는 `ANTHROPIC_API_KEY`) |
| **`TERM_SESSION_ID`** 환경 변수 | 세션별 상태 파일의 키로 사용됩니다. 대부분의 터미널 에뮬레이터(iTerm2, WezTerm, 최신 Linux 터미널)가 자동으로 설정합니다. 설정되지 않았을 때의 대안: `claude`를 실행하기 전에 `export TERM_SESSION_ID=$(uuidgen)`. |
...
```bash
brew install jq
# bash는 이미 설치되어 있습니다. 최신 bash 5.x가 필요하면: brew install bash
...
네이티브 Windows(PowerShell / cmd)는 **지원하지 않습니다**. 플러그인 전체가 bash 스크립트이며, Stop hook 등록이 `PATH` 안에 POSIX 쉘을 필요로 합니다. 두 가지 선택지가 있습니다.

- **WSL2 (권장)** — WSL2 배포판 안에서 Claude Code를 실행하세요. 모든 것이 그대로 동작합니다.
- **Git Bash (실험적)** — [Git for Windows](https://git-scm.com/download/win)를 설치하면 bash가 함께 설치됩니다. 그런 다음 `jq`를 별도로 설치하세요(예: [scoop](https://scoop.sh)을 통해: `scoop install jq`). 또한 `claude`를 실행하기 전에 `TERM_SESSION_ID`를 수동으로 export 해야 합니다:
  ```bash
...
| --- | --- |
| Linux | ✅ 테스트됨 |
| macOS | ✅ 동작할 것으로 예상 (동일한 POSIX 기반) |
| WSL2 on Windows | ✅ 테스트됨 |
| Git Bash on Windows | ⚠️ 실험적, `TERM_SESSION_ID` 수동 설정 필요 |
| 네이티브 Windows (PowerShell / cmd) | ❌ 지원 안 함 |
...
│   └── stop-hook.sh         # 핵심 루프 로직
├── scripts/
│   ├── setup-watchdog.sh    # 상태 파일 생성
│   └── stop-watchdog.sh     # 상태 파일 삭제
├── .gitattributes           # LF 줄바꿈 강제 (셸 스크립트에 필수)
...
| **상태 파일 포맷** | JSON (jq로 파싱) | YAML frontmatter가 있는 Markdown (sed/awk/grep으로 파싱) |
