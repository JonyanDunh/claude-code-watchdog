...
```bash
/watchdog:start "내가 /watchdog:stop할 때까지 계속 반복해." --no-classifier --max-iterations 0
```

Stop hook이 Haiku 호출을 **완전히 건너뜁니다**. 종료 수단은 `--max-iterations`와 `/watchdog:stop` 두 가지로 줄어듭니다. `--max-iterations 0`과 함께 쓰면 당신이 멈추라고 할 때까지만 도는 무제한 루프가 됩니다.
...
분류기 서브프로세스는 완벽하지 않습니다. 의미 없는 편집을 반복하며 멈추지 못하는 agent나, 헷갈려서 너무 일찍 편집을 중단해 버리는 agent는 결국 하드 스톱으로 떨어져야 합니다. `--max-iterations 20`이 합리적인 기본값입니다.
