...
| **`TERM_SESSION_ID`** 환경 변수 | 세션별 상태 파일의 키로 사용됩니다. iTerm2, WezTerm, Windows Terminal, 대부분의 최신 Linux 터미널이 자동으로 설정합니다. 설정되지 않았을 때의 대안: `claude`를 실행하기 전에 `export TERM_SESSION_ID=$(uuidgen)`. |
