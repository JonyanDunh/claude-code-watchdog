...
- **LLM 判决、面向"项目文件"的变更检测** —— 一次 headless `claude -p --model haiku` 调用是"当前 turn 有没有改项目文件"的**唯一判决者**。它能看到每个工具调用的完整 input，语义级判定。
...
| | Watchdog | ralph-loop |
| --- | --- | --- |
| **退出触发** | headless Haiku 分类器是**唯一判决者**。它能看到每个工具调用的完整 input，语义级判定任何项目文件有没有被直接修改。 | agent 必须在最终文本里吐一个 `<promise>…</promise>` XML tag。tag 里的短语通过 `--completion-promise "…"` 由用户设定（比如 `COMPLETE`、`DONE`），Stop hook 用 grep 做精确匹配。 |
