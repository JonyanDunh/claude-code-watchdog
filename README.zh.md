...
| **`bash`** 在 `PATH` 里 | 所有 hook 和 setup 逻辑都是 POSIX bash 写的。原生 Windows（PowerShell / cmd）**不支持** —— 用 WSL2 或 Git Bash |
...
| **`TERM_SESSION_ID`** 环境变量 | 作为 per-session 状态文件的 key。iTerm2、WezTerm、大多数现代 Linux 终端都会设置。如果没设：启动 `claude` 之前先 `export TERM_SESSION_ID=$(uuidgen)`。 |
...
```bash
brew install jq
# bash 系统自带；想用新版 bash 5.x 的话：brew install bash
...
原生 Windows（PowerShell / cmd）**不支持** —— 整个插件就是一坨 bash 脚本，Stop hook 的注册硬依赖 POSIX shell 在 `PATH` 里。两条路：

- **WSL2（推荐）** —— 在 WSL2 发行版里跑 Claude Code，开箱即用，啥都不用折腾。
- **Git Bash（实验性）** —— 装 [Git for Windows](https://git-scm.com/download/win)，它自带 bash，然后单独装 `jq`（比如用 [scoop](https://scoop.sh)：`scoop install jq`）。另外启动 `claude` 之前还得手动 export `TERM_SESSION_ID`：
  ```bash
...
| --- | --- |
| Linux | ✅ 已测试 |
| macOS | ✅ 应该能跑（同一套 POSIX 原语） |
| WSL2 on Windows | ✅ 已测试 |
| Git Bash on Windows | ⚠️ 实验性，需要手动设置 `TERM_SESSION_ID` |
