...
Watchdog 1.1.0 是**一次 Node.js 重写**。不用 bash，不用 jq，不用 POSIX coreutils —— 只要 `node` 和 `claude` CLI。Linux、macOS、Windows 原生跑，一个都不落下。

| 要求 | 原因 |
| --- | --- |
| **Claude Code 2.1+** | 使用 Stop hook 体系和 marketplace 插件格式 |
| **`node`** 18+ 在 `PATH` 里 | 所有 hook 和 setup 逻辑都是 JavaScript 写的。测试套件用的 `node:test` 也需要 Node 18+ |
| **`claude` CLI** 在 `PATH` 里 | 用于 headless Haiku 分类调用。必须已经完成认证（OAuth 或 `ANTHROPIC_API_KEY`） |
| **`TERM_SESSION_ID`** 环境变量 | 作为 per-session 状态文件的 key。iTerm2、WezTerm、大多数现代 Linux 终端都会设置。如果没设：启动 `claude` 之前先 `export TERM_SESSION_ID=$(node -e "console.log(require('crypto').randomUUID())")`。 |

### 安装依赖

如果你是用 `npm install -g @anthropic-ai/claude-code` 装的 Claude Code，`node` 已经在 `PATH` 里了，啥都不用再装。否则：
...
不用 WSL2，不用 Git Bash —— Watchdog 1.1.0 在原生 Windows 上直接跑。
...
| Windows（Node 18 / 20 / 22） | ✅ CI 已测试（原生 PowerShell / cmd，不需要 WSL2） |
| WSL2 on Windows | ✅ 能跑（本质就是 Linux） |
...
│   └── judge.js             # headless Haiku 子进程 + 判定解析
...
CI 会在 `ubuntu-latest`、`macos-latest`、`windows-latest` 三个平台上跨 Node 18 / 20 / 22 跑完整套件，每次 push 和 pull request 都会触发。
