...
_一个 Claude Code 插件：把当前 agent 困在同一会话里的自指循环中，不到任务真正停止产生文件编辑决不放手，没有"完成信号"，agent 没有任何方式能骗过循环提前溜走。_
...
这形成了一个 **self-referential feedback loop（自指反馈循环）**：
- 每一轮的 prompt 永远不变
- Claude 上一轮的工作保留在文件里
- 每一轮都能看到已经被修改的文件和 git 历史
- Claude 通过读自己过去的代码自主推进
