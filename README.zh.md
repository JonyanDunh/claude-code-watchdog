...
> **盯住 agent。识破谎言。只在真正完成时才停下。**

_一个 Claude Code 插件：把当前 agent 困在同一会话里的自指循环中，不到任务真正停止产生文件编辑决不放手。没有 `<promise>` 标签，没有"完成信号"，agent 没有任何方式能骗过循环提前溜走。_
...
就这样。Watchdog 会在每一轮 turn 结束后把你的 prompt 重新喂给 Claude，直到 Claude 要么：

- 某一轮完成时没有修改任何文件，**或者**
- 达到 `--max-iterations` 安全上限，**或者**
- 你手动运行 `/watchdog:stop`。

其他都是自动的。**Agent 从头到尾不知道自己在一个循环里**。

---

## 为什么用 Watchdog

- **零 agent 作弊空间** —— Agent 全程不知道自己在 loop 里。没有 `systemMessage`、没有迭代计数、没有启动 banner。它没有任何办法通过伪造完成信号来偷懒。
- **强制工具验证** —— 纯文本 turn（"我已经检查过了，没问题"）永远不会终止 loop。**必须**真的调用了工具，才有资格被考虑退出。
- **LLM 判决的"一切文件"变更检测** —— 一次 headless `claude -p --model haiku` 调用是"当前 turn 有没有改文件"的**唯一判决者**。它读当前 turn 的 tool 调用（工具名 + 如果是 Bash 就附带命令原文），语义级判定**任何**磁盘上的文件有没有被修改——常规文件、隐藏文件、dotfile、cache、lock、index、database、log、其他程序内部管理的文件、只改元数据（时间戳/权限）——**一个类别都不排除**。Hook 里**没有**任何硬编码 rubric、工具名白名单、bash 模式匹配器、"修改类命令"清单。判定权 100% 在 LLM 手里。
- **per-session 隔离** —— 状态文件按 `TERM_SESSION_ID` 分文件，在不同终端 tab 里并发跑多个 watchdog 互不干扰。
- **对 agent 完全隐藏** —— 所有诊断信息走 stderr，JSONL transcript 不会把 loop 元数据泄漏进 agent 的上下文。
- **Apache 2.0** —— 干净地派生自 Anthropic 官方的 `ralph-loop` 插件，完整归属写在 [NOTICE](./NOTICE) 里。
...
```text
1. /watchdog:start "<任务>"            用户只运行一次
2. Claude 开始做这个任务               读文件、编辑、写入、跑测试
3. Claude 的这一 turn 结束             助手自然停下
4. Stop hook 触发                     检查当前 turn 的 tool_use 块
5a. 调过工具 + 有文件改动              → 重发 prompt + 验证提醒 → 回到步骤 2
5b. 调过工具 + 没有文件改动            → 删除状态文件 → 会话正常退出
5c. 完全没调工具                       → 重发 prompt（必须证明验证）→ 回到步骤 2
```

这里的"循环"不是外部的——没有 `while true`、没有 orchestrator 进程。插件注册了一个 Stop hook，这个 hook 会阻止 Claude Code 的会话退出，并通过 Claude Code 原生的 `{"decision": "block", "reason": ...}` 协议把原始 prompt 作为新的 user turn 再注入回去。
...
| **Haiku 分类器判定** | 一次 headless `claude -p --model haiku` 调用返回 `NO_FILE_CHANGES`。分类器读当前 turn 的 `tool_use` 块（工具名 + Bash 命令原文），语义级判定**任何**磁盘文件有没有被修改，**涵盖所有类别**——常规、隐藏、cache、lock、index、内部、仅改元数据——**一个都不排除**。 |
...
按设计，**agent 不能知道自己在一个循环里**。如果它知道了，就会想办法第一轮就凭记忆伪造完成信号偷懒溜走。Watchdog 这样强制执行：
...
从 agent 视角看，就像同一个用户在反复问同一个问题，偶尔加一句"请你真的去重新检查一遍"。它看不到 Stop hook、看不到迭代计数、看不到任何 loop 元数据。**agent 骗不了它根本不知道存在的东西**。
...
❌ 不好：_"建一个 todo API，做得好点。"_

✅ 好：_"在 `src/api/todos.ts` 里实现 todo 的 REST API。要求：CRUD 端点齐全、有输入校验、`tests/todos.test.ts` 里覆盖率 80% 以上。一直迭代直到所有测试通过、覆盖率达标。"_

### 2. 可验证的、可逐步推进的目标

Loop 的退出条件是"没有文件被修改"。如果你的任务没有可验证的终止状态，它就会空转。

✅ 好：_"重构 `services/cache.ts`，删掉老的 LRU 实现。更新 `src/` 下所有调用方。每次修改后跑 `pnpm typecheck && pnpm test:cache`。迭代到两者都无警告通过为止。"_
...
- **每次迭代的 Haiku 成本** —— 每次 Stop hook 触发会花 ~10 秒 + 少量 token 在一次 headless `claude -p --model haiku` 调用上。这是 loop 延迟的主要来源。
- **同一个终端 tab 里开两个 `claude`** —— 它们会共享同一个 `TERM_SESSION_ID`，互相覆盖状态文件。用不同 tab 就行。
- **Hook 只在自然停下时触发** —— 如果某个 tool 调用搞崩了 Claude Code 本身，状态文件可能会残留。用 `/watchdog:stop` 清理即可。
- **失败方向是"继续 loop"** —— 如果 Haiku 调用失败（网络、认证、解析错误），loop 会继续而不是退出。宁可多转一轮也不丢工作。
...
| | Watchdog | ralph-loop |
| --- | --- | --- |
| **退出触发** | headless Haiku 分类器是**唯一判决者**，读当前 turn 的工具调用（包括 Bash 命令原文），语义级判定**任何**磁盘文件有没有被修改——universal 作用域、零类别排除。Hook 里**没有** rubric、**没有**工具名白名单、**没有**任何"修改类命令"清单。 | agent 必须在最终文本里吐一个 `<promise>…</promise>` XML tag。tag 里的短语通过 `--completion-promise "…"` 由用户设定（比如 `COMPLETE`、`DONE`），Stop hook 用 grep 做精确匹配。 |
