...
Stop hook 会**完全跳过** Haiku 调用。退出循环只剩两个途径：`--max-iterations` 和 `/watchdog:stop`。**`--max-iterations` 是可选的** —— 如果你完全不传(就像上面的示例)，循环就是真正的无限循环,只能你手动停。**不需要再写 `--max-iterations 0`** —— 直接不传这个参数就行(`0` 那种写法还能用,只是为了向后兼容)。
