...
| **`TERM_SESSION_ID`** 環境変数 | セッション単位の状態ファイルのキーになります。iTerm2、WezTerm、Windows Terminal、最近の Linux 端末の多くは自動で設定します。設定されていない場合の回避策: `claude` を起動する前に `export TERM_SESSION_ID=$(uuidgen)` を実行。 |
