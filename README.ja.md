...
```bash
/watchdog:start "私が /watchdog:stop するまで反復し続けて。" --no-classifier --max-iterations 0
```

Stop hook は Haiku 呼び出しを**完全にスキップ**します。終了する手段は `--max-iterations` と `/watchdog:stop` の 2 つだけになります。`--max-iterations 0` と組み合わせると、あなたが止めるまで永遠に走り続ける無制限ループになります。
