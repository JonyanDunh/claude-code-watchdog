...
### `--exit-confirmations` でより厳密な収束判定

デフォルトでは、Haiku 分類器が最初に `NO_FILE_CHANGES` の判定を返した瞬間にループは終了します。重要な作業で「本当に agent が収束したのか」を二重三重に確認したい場合は、しきい値を引き上げてください：

```bash
/watchdog:start "services/cache.ts をリファクタリング。pnpm test:cache が通るまで反復。" --exit-confirmations 3 --max-iterations 20
```

これでループは終了する前に**連続 3 ターン**のクリーンな判定を要求します。分類器が `NO_FILE_CHANGES` 以外の何か——`FILE_CHANGES`、`AMBIGUOUS`、分類器の失敗（`CLI_MISSING` / `CLI_FAILED`）、またはツール呼び出しのない純粋なテキストターン——を返した瞬間に streak カウンターは `0` にリセットされます。**収束が途切れていない**ことが必要です。

デフォルト値は `1` で、1.3.0 以前の動作と完全に同じです。**`--no-classifier` とは併用できません**。

### `--watch-prompt-file` でループ実行中に prompt をホットリロード

`--prompt-file` でループを開始した後、走っている最中にタスクを微調整したい場合は `--watch-prompt-file` を追加してください：

```bash
/watchdog:start --prompt-file ./tmp/task.md --watch-prompt-file --max-iterations 30
```

Stop hook は各イテレーションの開始時に prompt ファイルを再読み込みするようになります。前のターン以降に内容が変わっていた場合、新しいバージョンが次の user turn として使われ、**さらに** `--exit-confirmations` の streak カウンターが `0` にリセットされます（タスクが再定義された以上、古いタスクに対する収束を引き継ぐべきではありません）。

ホットリロードは**ループを絶対に壊しません**：hook が発火した時点でファイルが消えていたり、空だったり、読めなかったりした場合、キャッシュされた prompt が静かに維持され、ループは続行します。途中でファイルを編集したり、リネームしたり、一時的に移動したりしても何も壊れません——次のイテレーションはその時点でのファイルの状態を取り込みます。

`--prompt-file` が必要です。**`--watch-prompt-file` を単独で渡すとエラー**になります。

### `--no-classifier` で分類器を完全に無効化

ralph-loop スタイルの実行で「LLM に収束を判定させたくない」という場合——手動で停めるか `--max-iterations` で抜けるかのどちらか：

```bash
/watchdog:start "私が /watchdog:stop するまで反復し続けて。" --no-classifier
```

Stop hook は Haiku 呼び出しを**完全にスキップ**します。終了する手段は `--max-iterations` と `/watchdog:stop` の 2 つだけになります。**`--max-iterations` はオプション**です——上の例のように省略すれば、ループは本当に無制限になり、あなたが止めるまで動き続けます。「無制限」を意味するために `--max-iterations 0` を渡す必要は**もうありません**——フラグごと省略してください。（`0` の形式は後方互換のために引き続き受け付けます。）
...
### 4. ほとんどのタスクでは `--max-iterations` を設定する
...
| **収束の柔軟性** | `--exit-confirmations N` で終了前に **連続 N 回**のクリーンな `NO_FILE_CHANGES` 判定を要求できます（デフォルト 1）。`--no-classifier` は Haiku を完全にスキップし、`--max-iterations` または `/watchdog:stop` 経由でしか終了しない ralph-loop スタイルの実行を可能にします。 | `<promise>…</promise>` タグの発射＆ grep という単一のメカニズムのみで、調整可能な厳密度のつまみは一切なし —— agent が設定された promise フレーズを吐くか吐かないか、それだけです。 |
| **prompt の進化** | `--watch-prompt-file` は `--prompt-file` を毎イテレーションでホットリロードします。ループの実行中にタスク仕様を編集でき、次のターンが新しい内容を取り込みます（タスクが変わったので収束 streak もリセットされます）。ファイルが消えた / 空になった / 読めない場合はキャッシュした prompt を静かに維持 —— ホットリロードがループを壊すことは絶対にありません。 | prompt は `/ralph-loop "..."` の時点で固定され、ループを取り消して再起動しない限り変更**できません**。 |
