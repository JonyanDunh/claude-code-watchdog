...
| **`bash`** が `PATH` にあること | hook とセットアップのロジックはすべて POSIX bash で書かれている。ネイティブ Windows（PowerShell / cmd）は**サポートしない** —— WSL2 か Git Bash を使うこと |
| **`jq`** が `PATH` にあること | Stop hook がトランスクリプト JSONL と状態ファイル JSON のパースに使います |
| **`claude` CLI** が `PATH` にあること | ヘッドレスな Haiku 分類呼び出しに使います。認証済み（OAuth または `ANTHROPIC_API_KEY`）である必要があります |
| **`TERM_SESSION_ID`** 環境変数 | セッション単位の状態ファイルのキーになります。iTerm2、WezTerm、最近の Linux 端末の多くは自動で設定します。設定されていない場合の回避策: `claude` を起動する前に `export TERM_SESSION_ID=$(uuidgen)` を実行。 |
...
ネイティブ Windows（PowerShell / cmd）は**サポートしない**。プラグインは丸ごと bash スクリプトで、Stop hook の登録が POSIX シェルを `PATH` に要求する。選択肢は 2 つ。

- **WSL2（推奨）** —— WSL2 ディストロの中で Claude Code を走らせる。すべてそのまま動く。
- **Git Bash（実験的）** —— [Git for Windows](https://git-scm.com/download/win) をインストールすると bash が同梱されるので、別途 `jq` を入れる（例: [scoop](https://scoop.sh) で `scoop install jq`）。さらに `claude` を起動する前に `TERM_SESSION_ID` を手動で export する必要がある：
  ```bash
...
| --- | --- |
| Linux | ✅ テスト済み |
| macOS | ✅ 動くはず（同じ POSIX プリミティブ） |
| WSL2 on Windows | ✅ テスト済み |
| Git Bash on Windows | ⚠️ 実験的、`TERM_SESSION_ID` を手動で設定する必要あり |
| Native Windows (PowerShell / cmd) | ❌ サポートしない |
...
| **状態ファイルの形式** | JSON（jq でパース） | YAML frontmatter 付き Markdown（sed/awk/grep でパース） |
