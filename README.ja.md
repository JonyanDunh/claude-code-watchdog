...
Stop hook は Haiku 呼び出しを**完全にスキップ**します。終了する手段は `--max-iterations` と `/watchdog:stop` の 2 つだけになります。**`--max-iterations` はオプション**です——上の例のように省略すれば、ループは本当に無制限になり、あなたが止めるまで動き続けます。
